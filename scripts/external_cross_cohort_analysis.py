...
ROOT = Path(os.environ.get("AMA_PROJECT_ROOT", Path(__file__).resolve().parents[1])).resolve()
EXT = Path(os.environ.get("AMA_EXTERNAL_VALIDATION_DIR", ROOT / "external_validation")).resolve()
GSE23959_DIR = Path(os.environ.get("AMA_GSE23959_DIR", EXT / "GSE23959")).resolve()
OUT = Path(os.environ.get("AMA_OUTPUT_DIR", ROOT / "results" / "reproduced" / "external_validation")).resolve()
TABLES = OUT / "tables"
FIGS = OUT / "figures"
GENE_INFO = Path(os.environ.get("AMA_GENE_INFO", ROOT / "resources" / "Homo_sapiens.gene_info.gz")).resolve()
GSE23959_ANNOTATION_SQLITE = Path(
    os.environ.get(
        "AMA_GSE23959_SQLITE",
        ROOT / "resources" / "GSE23959_annotation" / "huex10sttranscriptcluster.sqlite",
    )
).resolve()
